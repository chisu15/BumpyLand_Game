...
namespace ActiveRagdoll {
    ...
    public class PhysicsModule : Module {
        ...
        private void ApplyUprightTorque() {
            var balancePercent = Vector3.Angle(_activeRagdoll.PhysicalTorso.transform.up, Vector3.up) / 180;
            balancePercent = uprightTorqueFunction.Evaluate(balancePercent);
            var rot = Quaternion.FromToRotation(_activeRagdoll.PhysicalTorso.transform.up, Vector3.up).normalized;
        ...
        private void FreezeRotation() {
            var smoothedRot = Quaternion.Lerp(_activeRagdoll.PhysicalTorso.rotation, _targetRotation, Time.fixedDeltaTime * freezeRotationSpeed);
            _activeRagdoll.PhysicalTorso.MoveRotation(smoothedRot);
        }

        private void StabilizeWithJoint() {
            _stabilizerRigidbody.MovePosition(_activeRagdoll.PhysicalTorso.position);
            _stabilizerRigidbody.MoveRotation(_targetRotation);
        }

        private void ApplyManualTorque() {
        ...
        /// <summary>
        /// Prepares the joints for jumping by freeing angular constraints.
        /// </summary>
        public void PrepareForJump() {
            foreach (var joint in _activeRagdoll.Joints) {
                joint.angularXMotion = ConfigurableJointMotion.Free;
                joint.angularYMotion = ConfigurableJointMotion.Free;
                joint.angularZMotion = ConfigurableJointMotion.Free;
            }
            Debug.Log("Joints prepared for jump.");
        }

        /// <summary>
        /// Resets the joints to their original constrained state after jumping.
        /// </summary>
        public void ResetAfterJump() {
            ...
            Debug.Log("Joints reset after jump.");
        }
    }
} // namespace ActiveRagdoll
